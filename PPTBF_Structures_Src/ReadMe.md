...
Currently, it works on ***Windows*** operating systems. A Linux port has been added but not yet its documentation (just use ".sh" scripts instead of ".bat" ones).
- [x] Windows 10
...
- for instance, to select *Visual Studio 2017 x64 version*, one can use: ***cmake -G "Visual Studio 15 2017 Win64" ..\glad***. But by default, not using a dedicated generator, CMake will find the first available generator in the PATH environment variable: ***cmake ..\glad***
...
```
  ...
  cmake -G %GS_COMPILER% ..
```
  - as mentioned, removing the *-G* option and its generator name *should work* (read above to check for the 32 bits vs 64 bits version of Visual Studio: it should be visible in the CMake LOG info at the beginning of the process), and CMake will look at your (latest installed) compiler/IDE in PATH environment variable:
```
  cmake ..
```
